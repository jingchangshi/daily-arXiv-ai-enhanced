...
TL;DR: LOOPRAG：基于检索增强生成的循环优化框架，通过参数驱动方法生成多样化合法示例，结合循环感知检索和反馈迭代机制，显著提升LLMs在循环变换优化中的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管循环变换是广泛使用的语义保持优化技术，但找到最优的循环变换组合仍然具有挑战性。现有大型语言模型在循环优化中经常产生错误或次优结果，错失性能提升机会。

Method: 提出LOOPRAG框架：1）参数驱动方法利用循环属性触发各种循环变换，生成多样化合法示例代码；2）基于循环特征的循环感知检索算法，平衡相似性和多样性；3）反馈式迭代机制，结合编译、测试和性能结果指导LLMs；4）通过变异、覆盖和差异测试进行等价性检查。

Result: 在PolyBench、TSVC和LORE基准测试中，相比基础编译器分别获得最高11.20×、14.34×和9.29×的加速比，相比基础LLMs分别获得最高11.97×、5.61×和11.59×的加速比。

Conclusion: LOOPRAG框架有效解决了LLMs在循环优化中的局限性，通过检索增强生成和反馈迭代机制显著提升了循环变换优化的性能，在多个基准测试中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 自主自适应系统在软件开发中面临重大挑战，特别是需要明确捕捉环境或决策过程中的不确定性。安全关键和任务关键系统必须经过严格审查，但开发者直接使用复杂的概率逻辑形式化规范是不现实且容易出错的

Method: 扩展NASA的FRET工具的结构化自然语言，支持概率需求规范；开发形式化、组合式、自动化的方法，将结构化自然语言需求转换为概率时序逻辑公式；建立自动化验证框架和形式化证明来确保生成公式的正确性

Result: 扩展后的FRET工具使开发者能够用结构化自然语言指定概率需求，并自动转换为概率时序逻辑公式，使自主自适应系统的形式化分析更加实用且减少错误

Conclusion: 通过扩展FRET工具支持概率需求规范，并开发自动化转换和验证方法，显著提高了自主自适应系统形式化分析的可行性和可靠性
...
TL;DR: NeuroInv是一个神经符号方法，用于生成循环不变量，结合LLM的推理能力和符号验证，在Java程序验证中达到99.5%的成功率。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的循环不变量生成方法缺乏可靠的结构化方法论，且很少参考现有的程序验证理论。循环不变量生成仍然是自动化程序验证的关键瓶颈。

Method: NeuroInv包含两个关键模块：(1) 神经推理模块：利用LLM和霍尔逻辑通过后向链最弱前置条件推理来推导和精化候选不变量；(2) 验证引导的符号模块：使用OpenJML的反例迭代修复不变量。

Result: 在包含150个Java程序的综合基准测试中，NeuroInv达到99.5%的成功率，显著优于其他评估方法。在包含10个更大规模多循环程序（平均每个程序7个循环）的困难基准测试中，NeuroInv也表现出色，证明其能够扩展到更复杂的验证场景。

Conclusion: NeuroInv通过结合神经推理和符号验证的方法，为循环不变量生成提供了一个可靠且结构化的解决方案，能够有效处理复杂的程序验证任务。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型越来越依赖外部工具来增强能力，但工具调用在推理过程中引入了性能瓶颈，影响系统吞吐量

Method: 提出两种系统优化：1) 推测工具调用，提前预测并执行可能的工具调用；2) 强制序列在推理引擎中保持驻留以减少开销

Result: 优化使LM代理推理吞吐量提升数百token/秒，并提供理论分析指导最佳推测配置

Conclusion: 提出的优化方案能有效解决工具调用性能瓶颈，建议LM提供商采用新的"工具缓存"API端点来轻松实施这些优化
...
TL;DR: LOG.io是一个用于分布式数据流水线的日志式回滚恢复和细粒度数据血缘捕获系统，在存在慢速算子时表现优于ABS协议


<details>
  <summary>Details</summary>
Motivation: 分布式数据流水线需要可靠的故障恢复机制和细粒度数据血缘追踪，现有解决方案如Flink的ABS协议在处理非确定性算子、外部系统交互和慢速算子时存在局限性

Method: 采用日志式回滚恢复协议，支持非确定性算子、外部系统交互和任意自定义代码，实现非阻塞恢复和算子独立恢复，支持流水线执行期间的动态算子扩展

Result: 在存在慢速算子且事件吞吐量适中时，LOG.io在正常处理中表现与ABS相当，在恢复时优于ABS；其他情况下ABS表现更好，但数据并行化能显著减少LOG.io开销；细粒度数据血缘捕获开销小于1.5%

Conclusion: LOG.io为分布式数据流水线提供了有效的回滚恢复和细粒度数据血缘捕获解决方案，特别适用于存在慢速算子的场景，且数据血缘捕获开销可忽略不计
...
<details>
  <summary>Details</summary>
Motivation: PCIe带宽已成为大语言模型性能的关键瓶颈，特别是在前缀缓存获取和模型切换场景中。当前异构协议（PCIe和NVLink）限制了主机内存与GPU之间的带宽，导致服务器内部带宽未充分利用。

Method: 提出多路径内存访问（MMA）方案，首次实现GPU与主机内存间的高效多路径数据传输。通过动态库注入实现无缝部署，无需修改LLM应用程序代码。

Result: 在测试平台上，MMA显著提升GPU与内存间数据传输带宽，峰值带宽达245 GB/s，相比原生单路径带宽提升4.62倍。端到端评估显示，MMA将LLM服务的首token时间减少1.14-2.38倍，并将vLLM睡眠模式下的模型切换延迟降低1.12-2.48倍。

Conclusion: MMA有效解决了PCIe带宽瓶颈问题，通过多路径数据传输技术充分利用了服务器内部带宽，显著提升了大语言模型服务的性能，特别是首token响应时间和模型切换效率。
...
TL;DR: 提出目标导向语义孪生(GOST)框架，替代传统数字孪生，在空天地海一体化网络中优先考虑"效用"而非"保真度"，实现轻量级、任务特定的表示


<details>
  <summary>Details</summary>
Motivation: 传统数字孪生在空天地海一体化网络中面临计算开销大、模型同步延迟、跨系统语义鸿沟等根本性限制，无法满足实时态势感知和智能运维维护的需求

Method: 提出目标导向语义孪生(GOST)框架，包含知识驱动语义、数据驱动语义和目标导向原则三层结构，利用语义技术和目标导向原则构建轻量级、任务特定的表示

Result: 在远程卫星-无人机网络协同跟踪案例研究中，GOST在感知数据时效性和协同跟踪方面显著优于传统数字孪生

Conclusion: GOST作为一种变革性的孪生范式，能够指导空天地海一体化网络的发展，为解决传统数字孪生在复杂网络环境中的局限性提供了有效方案
...
TL;DR: 该论文将冷启动问题视为开发者可见的设计问题，通过分析81个开源服务器无系统问题报告，提出初始化反模式分类、修复策略和诊断挑战，并开发了SCABENCH基准测试和INITSCOPE分析框架，显著提升了诊断准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 服务器无计算简化了部署和扩展，但冷启动延迟仍然是主要的性能瓶颈。现有研究将缓解措施视为黑盒优化，而本文认为冷启动应作为开发者可见的设计问题来研究。

Method: 1. 分析81个开源服务器无系统的问题报告，推导初始化反模式、修复策略和诊断挑战的分类体系；2. 开发SCABENCH可重现基准测试；3. 提出INITSCOPE轻量级分析框架，将加载的代码与执行的代码关联起来。

Result: 在SCABENCH上，INITSCOPE相比现有工具将定位准确率提升高达40%，诊断工作量减少64%。开发者研究表明，使用该工具的任务准确率更高，诊断速度更快。

Conclusion: 该研究为服务器无设计中的冷启动缓解提供了基于证据、性能感知的实践方法，研究工件已公开可用，支持未来研究和改进。
...
Method: 提出在线调度框架，整合条件负载均衡、动态分区和作业迁移技术，动态调整作业放置以最小化争用，并重新组织GPU分配以解决内部和外部碎片化

Result: 实验结果显示该方法显著提升系统效率，当所有技术都应用时，makespan（总完成时间）改善高达35%

Conclusion: 提出的在线调度框架能有效解决MIG GPU的资源争用和碎片化问题，显著提升GPU资源共享的系统效率
...
TL;DR: 提出Staggered Batch Scheduling (SBS)调度机制，通过缓冲请求形成最优执行批次，解决DP+EP架构中的内部同步成本问题，显著降低TTFT并提升吞吐量


<details>
  <summary>Details</summary>
Motivation: 随着LLM服务向复杂的分布式架构（特别是P/D分离的大规模DP+EP范式）演进，引入了独特的调度挑战。与传统部署中调度器可以将实例视为黑盒不同，DP+EP架构表现出高昂的内部同步成本。立即请求调度会导致严重的引擎内排队和并行化气泡，降低首令牌时间(TTFT)

Method: 提出Staggered Batch Scheduling (SBS)机制，通过故意缓冲请求来形成最优执行批次，实现时间解耦以消除内部排队气泡。利用缓冲创建的调度窗口，引入负载感知全局分配策略，在预填充和解码阶段平衡DP单元间的计算负载

Result: 在生产H800集群上部署服务Deepseek-V3，与最先进的立即调度基线相比，系统将TTFT降低了30%-40%，吞吐量提高了15%-20%

Conclusion: SBS调度机制有效解决了DP+EP架构中的调度挑战，通过缓冲和负载感知分配策略，在保持吞吐量的同时显著改善了首令牌时间，为大规模分布式LLM服务提供了高效的调度解决方案
...
TL;DR: Lotus提出了一种在分解内存架构上实现锁分解的分布式事务系统，通过将锁管理从内存节点转移到计算节点来解决RDMA网卡瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有分解内存上的分布式事务系统中，内存节点的RDMA网卡因处理大量单向原子锁操作而成为性能瓶颈，限制了系统的可扩展性。

Method: 1) 锁分解：将锁从数据中分离，全部在计算节点执行；2) 应用感知的锁管理机制：利用OLTP工作负载的局部性进行锁分片并保持负载均衡；3) 锁优先事务协议：将锁获取作为读写事务执行的第一步；4) 无锁重建恢复机制：将锁视为临时状态，避免重建。

Result: 实验结果表明，与现有分解内存上的事务系统相比，Lotus将事务吞吐量提高了最多2.1倍，延迟降低了最多49.4%。

Conclusion: Lotus通过锁分解有效解决了分解内存架构中RDMA网卡的瓶颈问题，实现了高可扩展的分布式事务处理，显著提升了系统性能。
...
TL;DR: FlexKV是一个内存解耦的键值存储系统，通过索引代理技术将索引动态卸载到计算节点，解决了现有系统对单边原子操作的过度依赖和计算侧缓存效率受限的问题，性能提升显著。


<details>
  <summary>Details</summary>
Motivation: 现有内存解耦键值存储系统存在两个关键问题：1）索引处理过度依赖单边原子操作；2）计算侧缓存效率受限。这些问题导致性能不佳，需要新的解决方案来提升内存解耦架构下的KV存储性能。

Method: FlexKV采用索引代理技术，将索引动态卸载到计算节点，利用其强大的CPU加速索引处理。具体包括：1）基于秩感知的热度检测算法平衡索引负载；2）两级计算节点内存优化方案；3）RPC聚合的缓存管理机制减少缓存一致性开销。

Result: 实验结果显示，与最先进的内存解耦KV存储相比，FlexKV将吞吐量提升高达2.94倍，延迟降低高达85.2%。

Conclusion: FlexKV通过索引代理技术有效解决了内存解耦KV存储的性能瓶颈，显著提升了系统性能，为内存解耦架构下的数据存储提供了高效解决方案。
...
TL;DR: 本文介绍了一个专为科学AI工作负载设计的联邦计算平台，提供从模型开发、训练到部署的完整ML生命周期支持，具有可重复部署、透明访问分布式基础设施等特点。


<details>
  <summary>Details</summary>
Motivation: 科学AI工作负载需要可重复、透明的分布式计算资源访问，以及覆盖完整机器学习生命周期的集成体验，以支持科学研究和AI模型开发。

Method: 构建一个联邦计算平台，通过可重复部署提供对物理分布式e-Infrastructures的一致透明访问，包含全面的服务目录，支持模型开发（交互式开发环境）、训练（GPU资源、标注工具、实验跟踪、联邦学习）和部署（云连续体上的多种部署选项）。

Result: 平台提供了完整的ML生命周期集成体验，包括AI模型可追溯性和可重复性工具，与不同AI模型提供商、数据集和存储资源集成，允许用户与更广泛的机器学习生态系统交互。

Conclusion: 该平台易于定制，能够降低外部社区的采用门槛，为科学AI工作负载提供了一个全面、可重复、可扩展的联邦计算解决方案。
...
TL;DR: 提出一种CPU-GPU协同推理框架，通过GPU专家缓存机制减少数据传输，利用CPU多线程处理缓存未命中，提升消费级硬件上MoE模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型计算需求高，MoE模型虽通过选择性激活参数降低计算量，但仍需大量内存超出消费级GPU容量。传统卸载方法在CPU和GPU间传输权重会引入延迟，限制推理性能。

Method: 提出新颖的CPU-GPU协同推理框架：1) GPU上实现专家缓存机制减少数据传输；2) 通过缓存命中实现快速推理；3) 将计算卸载到CPU高效处理缓存未命中；4) 利用CPU多线程优化。

Result: 评估显示该框架带来性能提升，突显了CPU-GPU协作在消费级系统单请求推理场景中最大化硬件利用的潜力。
...
TL;DR: 提出多阶段边缘服务器升级（M-ESU）问题，通过多阶段规划优化MEC系统升级，包括服务器部署/升级决策和任务卸载优化，以最大化满足延迟要求的任务数量。


<details>
  <summary>Details</summary>
Motivation: 现有的多接入边缘计算（MEC）系统需要长期升级以适应任务增长、延迟要求变严格等变化，但缺乏考虑多阶段预算约束和成本折旧的系统规划方法。

Method: 提出M-ESU框架，包含混合整数线性规划（MILP）模型和高效启发式算法（M-ESU/H）。MILP用于小型网络求最优解，M-ESU/H用于大规模网络，考虑预算、成本折旧、服务器资源、任务增长等多重约束。

Result: 小型网络中，M-ESU/H解与最优解差距在1.25%内，运行速度快几个数量级。大型网络中，相比仅考虑服务器部署或优先部署/升级的启发式算法，M-ESU/H在相同预算和需求增长下任务满足率提升高达21.57%。

Conclusion: M-ESU框架为长期MEC系统规划提供了可扩展的实用解决方案，能有效处理多阶段升级决策，平衡服务器部署/升级与任务卸载优化，显著提升系统性能。
...
TL;DR: 提出基于分支工作集大小和可预测性的分支预测性能分析方法，通过分析分支上下文模式来评估现代分支预测器的准确性。


<details>
  <summary>Details</summary>
Motivation: 分支预测是模式识别问题，准确的分支预测能减少错误路径指令执行，提升性能和降低能耗。需要一种系统的方法来分析和表征分支预测的工作负载特征。

Method: 提出分支工作集大小和分支可预测性两个新的工作负载驱动标识符。分支工作集定义为最常出现的分支上下文（分支地址、全局历史、局部历史的三元组）的集合。分析了2451个工作负载轨迹，将其分为7个分支工作集大小类别和9个可预测性类别。

Result: 提出的分支工作集大小和可预测性参数与现代分支预测方案（如TAGE和感知器）的误预测率高度相关。通过分析分支行为模式，揭示了预测准确性的来源和现代分支预测器偏好的工作负载类别。

Conclusion: 建立了一种系统的工作负载表征方法学，通过分支工作集分析为分支预测性能评估提供了新的视角，有助于理解现代分支预测器的性能特征和工作负载适应性。
...
TL;DR: 论文提出了Aria2可穿戴上下文AI系统的完整架构视图，强调端到端系统建模的重要性，以解决复杂系统设计和严格功耗约束的挑战。


<details>
  <summary>Details</summary>
Motivation: 下一代人机交互需要始终在线、空间感知的可穿戴设备来捕获自我中心视觉和功能原语，构建用户个人上下文以支持上下文AI助手。然而，设计这样的系统面临复杂性和严格功耗约束的挑战。

Method: 提出了Aria2可穿戴上下文AI系统的完整系统架构视图，通过系统建模和设计空间探索过程，展示了端到端全系统模型的重要性。

Result: 研究表明，在这种系统中，没有任何单一组件或类别完全主导系统功耗，这意味着需要在全系统上下文中做出长期设计决策和功耗优化，以避免其他系统瓶颈造成的限制。

Conclusion: 端到端全系统视图对于可穿戴上下文AI系统设计至关重要，需要在整个系统背景下进行优化决策，这些见解对于最终实现全天候可穿戴上下文AI系统具有重要意义。
