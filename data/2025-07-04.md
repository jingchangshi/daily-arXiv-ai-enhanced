...
### [2] [DecoRTL: A Run-time Decoding Framework for RTL Code Generation with LLMs](https://arxiv.org/abs/2507.02226)
*Mohammad Akyash,Kimia Azar,Hadi Kamali*
...
Method: DecoRTL结合了自一致性采样和语法感知温度适应，通过生成多个候选代码并根据语法和功能角色调整采样温度。
...
### [3] [SAKURAONE: Empowering Transparent and Open AI Platforms through Private-Sector HPC Investment in Japan](https://arxiv.org/abs/2507.02124)
*Fumikazu Konishi*
...
### [4] [Signalling Health for Improved Kubernetes Microservice Availability](https://arxiv.org/abs/2507.02158)
...
### [5] [Domain-Adversarial Transfer Learning for Fault Root Cause Identification in Cloud Computing Systems](https://arxiv.org/abs/2507.02233)
...
### [6] [Flotilla: A scalable, modular and resilient federated learning framework for heterogeneous resources](https://arxiv.org/abs/2507.02295)
...
### [7] [Alps, a versatile research infrastructure](https://arxiv.org/abs/2507.02404)
...
### [8] [FlowSpec: Continuous Pipelined Speculative Decoding for Efficient Distributed LLM Inference](https://arxiv.org/abs/2507.02620)
...
### [9] [SAKURAONE: Empowering Transparent and Open AI Platforms through Private-Sector HPC Investment in Japan](https://arxiv.org/abs/2507.02124)
*Fumikazu Konishi*

Main category: cs.DC

TL;DR: SAKURAONE是一个高性能计算集群，采用开放网络技术，全球排名49，专为大型语言模型训练等高级工作负载优化。
...
Method: 采用800 GbE和SONiC操作系统构建开放网络，配备NVIDIA H100 GPU和全闪存Lustre存储，支持高性能并行计算。

Result: HPL基准测试中达到33.95 PFLOP/s，HPCG中396.295 TFLOP/s，FP8精度下AI应用性能达339.86 PFLOP/s。
...
### [10] [Signalling Health for Improved Kubernetes Microservice Availability](https://arxiv.org/abs/2507.02158)
*Jacob Roberts,Blair Archibald,Phil Trinder*
...
<details>
  <summary>Details</summary>
Motivation: 微服务的高可用性对许多应用至关重要，但传统的PCM方法存在检测慢、误报等问题，需要更高效的监控方案。

Method: 设计并实现了基于Kubernetes的SCM方法，通过实验和数学模型比较SCM与PCM的性能。

Result: SCM检测容器故障速度快86%，且无需调优，资源开销有限；PCM误报导致可用性降低4%。

Conclusion: 建议容器编排器支持SCM功能，以更快、更准确地检测故障。
...
### [11] [Domain-Adversarial Transfer Learning for Fault Root Cause Identification in Cloud Computing Systems](https://arxiv.org/abs/2507.02233)
*Bruce Fang,Danyi Gao*
...
Method: 采用迁移学习，引入共享特征提取模块和领域对抗机制，结合伪标签选择策略增强模型能力。

Result: 在标签稀缺、类别不平衡和异构节点环境下，模型在准确性、F1-Score和AUC等指标上优于主流方法。

Conclusion: 该方法在复杂云计算系统中表现出高效性和鲁棒性，验证了其实际应用价值。
...
### [12] [Flotilla: A scalable, modular and resilient federated learning framework for heterogeneous resources](https://arxiv.org/abs/2507.02295)
*Roopkatha Banerjee,Prince Modi,Jinal Vyas,Chunduru Sri Abhijit,Tejus Chandrashekar,Harsha Varun Marisetty,Manik Gupta,Yogesh Simmhan*
...
<details>
  <summary>Details</summary>
Motivation: 随着移动和边缘计算的进步以及对数据隐私的关注，联邦学习（FL）作为一种隐私保护的分布式机器学习方法迅速流行。然而，现有框架多用于伪分布式模拟，缺乏对真实边缘硬件部署和异步聚合的支持。

Method: Flotilla采用模块化设计，支持快速组合多种FL策略，独立于DNN架构。它使用无状态客户端和分离会话状态的服务器设计，支持定期或增量检查点。

Result: Flotilla在200+客户端上展示了故障容忍能力，并在Raspberry Pi和Nvidia Jetson上的资源使用优于现有框架（Flower、OpenFL、FedML）。它还支持1000+客户端的扩展。

Conclusion: Flotilla是一个有竞争力的FL框架，适合构建新策略、快速部署和系统优化研究。
...
### [13] [Alps, a versatile research infrastructure](https://arxiv.org/abs/2507.02404)
*Maxime Martinasso,Mark Klein,Thomas C. Schulthess*

Main category: cs.DC

TL;DR: CSCS开发了Alps，一种新型HPC基础设施，通过独立端点资源和模块化设计满足多样化科学需求。
...
Method: Alps采用异构硬件（CPU和GPU）和高性能Slingshot网络，结合软件定义的vCluster技术，实现资源独立和模块化服务。

Result: Alps支持多种科学领域，如数值天气预报和AI研究，提供定制化平台。

Conclusion: Alps通过创新架构和技术，成功解决了传统HPC的局限性，为多样化科学需求提供了灵活支持。
...
### [14] [FlowSpec: Continuous Pipelined Speculative Decoding for Efficient Distributed LLM Inference](https://arxiv.org/abs/2507.02620)
*Xing Liu,Lizhuo Luo,Ming Tang,Chao Huang*

Main category: cs.DC

TL;DR: FlowSpec是一个基于流水线并行和树状推测解码的框架，旨在提升边缘网络中大型语言模型（LLM）的分布式推理效率。通过优化验证、管理和扩展策略，显著提高了推理速度。


<details>
  <summary>Details</summary>
Motivation: 边缘网络中稀疏的推理请求导致现有流水线方法利用率低，影响推理效率。FlowSpec旨在解决这一问题。

Method: FlowSpec结合了三种关键机制：基于分数的逐步验证、高效的草稿管理和动态草稿扩展策略。

Result: 实验表明，FlowSpec在多种模型和配置下显著提升推理速度，速度比基线快1.36倍至1.77倍。
...
### [15] [Advanced Printed Sensors for Environmental Applications: A Path Towards Sustainable Monitoring Solutions](https://arxiv.org/abs/2507.02067)
...
### [16] [Hardware-Accelerated Algorithm for Complex Function Roots Density Graph Plotting](https://arxiv.org/abs/2507.02164)
...
### [17] [System-performance and cost modeling of Large Language Model training and inference](https://arxiv.org/abs/2507.02456)
...
### [18] [AC-Refiner: Efficient Arithmetic Circuit Optimization Using Conditional Diffusion Models](https://arxiv.org/abs/2507.02598)
...
### [19] [Breaking the HBM Bit Cost Barrier: Domain-Specific ECC for AI Inference Infrastructure](https://arxiv.org/abs/2507.02654)
...
### [20] [Advanced Printed Sensors for Environmental Applications: A Path Towards Sustainable Monitoring Solutions](https://arxiv.org/abs/2507.02067)
*Nikolaos Papanikolaou,Doha Touhafi,Jurgen Vandendriessche,Danial Karimi,Sohail Fatimi,Gianluca Cornetta,Abdellah Touhafi*
...
Result: 传感器能够高效检测污染物、温度变化、湿度等关键参数，适用于环境评估和保护。

Conclusion: 印刷传感器在环境监测等领域展现出巨大潜力，为传感器技术提供了新的发展方向。
...
### [21] [Hardware-Accelerated Algorithm for Complex Function Roots Density Graph Plotting](https://arxiv.org/abs/2507.02164)
*Ruibai Tang,Chengbin Quan*

Main category: cs.AR

TL;DR: 提出了一种基于FPGA的硬件加速算法，用于高效绘制复函数根的密度图，通过多项式逼近和单位移QR迭代实现。
...
Conclusion: 该算法在能效上表现优异，适合大规模多项式处理，但受限于FPGA的制造技术。
...
### [22] [System-performance and cost modeling of Large Language Model training and inference](https://arxiv.org/abs/2507.02456)
*Wenzhe Guo,Joyjit Kundu,Uras Tos,Weijiang Kong,Giuliano Sisto,Timon Evenblij,Manu Perumkunnil*

Main category: cs.AR

TL;DR: 提出了一种性能-成本建模方法，用于优化大型语言模型（LLM）的训练和推理，结合了计算、内存和通信技术的最新进展。


<details>
  <summary>Details</summary>
Motivation: LLM的规模和复杂性增长过快，超过了计算能力、内存带宽和网络性能的提升，导致分布式系统扩展面临挑战。

Method: 整合了先进的计算技术、内存优化和通信技术，包括闪存注意力技术和专家混合模型，并分析了不同网络拓扑和通信算法的影响。

Result: 提出的建模方法能够分析性能-成本权衡，为未来计算系统设计和软硬件协同开发提供指导。

Conclusion: 该框架为LLM的扩展提供了有价值的见解，特别是在性能与成本之间的权衡分析方面。
...
### [23] [AC-Refiner: Efficient Arithmetic Circuit Optimization Using Conditional Diffusion Models](https://arxiv.org/abs/2507.02598)
*Chenhao Xue,Kezhi Li,Jiaxing Zhang,Yi Ren,Zhengyuan Shi,Chen Zhang,Yibo Lin,Lining Zhang,Qiang Xu,Guangyu Sun*

Main category: cs.AR

TL;DR: AC-Refiner提出了一种基于条件扩散模型的算术电路优化框架，通过将电路合成任务转化为条件图像生成任务，显著提升了设计质量。
...
Method: AC-Refiner利用条件扩散模型，将电路合成任务转化为条件图像生成，并通过目标质量结果（QoR）指导去噪过程。

Result: 实验表明，AC-Refiner生成的电路设计在Pareto最优性上优于现有方法，并在实际应用中验证了其性能提升。

Conclusion: AC-Refiner通过条件扩散模型有效优化算术电路设计，为高性能、低功耗和面积优化的电路提供了新思路。
...
### [24] [Breaking the HBM Bit Cost Barrier: Domain-Specific ECC for AI Inference Infrastructure](https://arxiv.org/abs/2507.02654)
*Rui Xie,Asad Ul Haq,Yunhua Fang,Linsen Ma,Sanchari Sen,Swagath Venkataramani,Liu Liu,Tong Zhang*

Main category: cs.AR

TL;DR: 论文提出一种系统级方法，通过将HBM的故障管理从片上ECC转移到内存控制器，结合特定领域的ECC框架，显著降低成本，同时在高误码率下保持高性能和模型精度。


<details>
  <summary>Details</summary>
Motivation: HBM的高成本限制了其在AI领域的可扩展部署，尤其是片上ECC的严格要求。

Method: 引入领域特定的ECC框架，结合大码字RS纠错、轻量级CRC检测、差分奇偶更新和可调保护机制。

Result: 在误码率高达10^-3时，系统仍能保持78%的吞吐量和97%的模型精度。
