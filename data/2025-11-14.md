...
TL;DR: Cyclotron是一个用于表达流式数据流算法的框架和编译器，通过递归方程将算法可移植地编译到分布式处理器拓扑中，优化内存访问并支持流水线执行。


<details>
  <summary>Details</summary>
Motivation: 为了解决流式数据流算法在分布式硬件上的高效执行问题，提供一种能够优化内存访问、支持流水线执行的可移植编译框架。

Method: 使用递归方程表达算法，通过中间表示层将逻辑张量递归降低到逻辑迭代空间递归，最终生成针对每个处理器的发送、接收和计算操作程序。

Result: 在可重构模拟器和分布式CPU集群上验证了框架的可移植性，生成的矩阵乘法和三角求解分布式实现性能与ScaLAPACK相当。

Conclusion: Cyclotron框架成功实现了流式数据流算法在分布式硬件上的高效可移植编译，为硬件设计空间探索和分布式计算提供了有效工具。
...
TL;DR: 本文提出了全向类型推断方法，通过动态信息流和挂起匹配约束来解决ML类型系统扩展中的主类型性问题，比现有方法更灵活且表达能力更强。


<details>
  <summary>Details</summary>
Motivation: ML类型系统的许多扩展（如GADTs、高阶多态、静态重载）破坏了主类型性，导致类型推断变得脆弱。现有的双向类型推断方法采用固定的推断顺序，经常拒绝本应类型正确的程序。

Method: 提出全向类型推断，允许类型信息以动态顺序流动，使用挂起匹配约束来暂停和恢复类型约束的求解。引入增量实例化机制来处理let泛化问题。

Result: 该方法在OCaml的两个重要特性（记录标签和数据构造器的静态重载、半显式一等多态）上都实现了主类型推断，比OCaml当前类型检查器表达能力更强。

Conclusion: 全向性为恢复脆弱特性中的主类型性提供了通用框架，能够处理ML类型系统扩展带来的挑战，提供更灵活和强大的类型推断能力。
...
TL;DR: 提出了Linear Core系统，在线性类型语言中引入惰性语义的线性性概念，解决Haskell优化编译器中语法线性性被破坏但语义保持的问题。


<details>
  <summary>Details</summary>
Motivation: 传统线性类型语言中，消耗线性资源等同于其在程序中的语法出现。但在非严格求值下，语法出现不一定意味着实际使用资源。Haskell优化编译器会重写程序，破坏语法线性性但保持语义，这需要新的线性性处理方法。

Method: 设计Linear Core系统，在Haskell的Core中间语言基础上引入静态惰性语义线性性，证明系统正确性并验证优化转换保持线性性。

Result: Linear Core系统被证明是可靠的，能保证线性资源使用，且多个优化转换在Linear Core中保持线性性而在Core中失败。已实现为编译器插件并在线性密集型库中验证。

Conclusion: Linear Core成功解决了惰性语言中线性性与编译器优化的冲突，为Haskell等语言提供了实用的线性类型系统实现方案。
...
TL;DR: 本文通过整数集合库(ISL)为CuTe和Triton两种布局系统建立了统一的数学表示，实现了形式化分析和正确性验证。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习编译器依赖布局抽象来管理逻辑张量结构与物理内存安排之间的复杂映射。CuTe布局和Triton线性布局是广泛采用的行业标准，但它们具有不同的数学基础，阻碍了统一的形式化分析和跨系统推理。

Method: 使用整数集合库(ISL)通过整数集合关系为两种布局系统创建统一的数学表示：CuTe布局通过基于步长的计算建模从多维坐标到线性索引的转换，包括用于增强内存访问模式的位级操作；Triton线性布局则建模遵循有限域F_2规则的二进制向量空间变换。

Result: 实现了完整的布局操作算法套件，包括组合、反转、补集等操作，确保数学正确性和布局语义保持。实验评估表明系统能够处理从基本恒等变换到复杂多维张量安排的全谱系布局复杂度。

Conclusion: 该方法成功地为不同布局范式建立了统一的数学建模方法，为未来的跨系统优化策略奠定了基础。
...
TL;DR: zkStruDul是一个统一输入转换和谓词定义的语言，通过单一抽象消除重复代码和安全不匹配问题，支持递归证明等特性


<details>
  <summary>Details</summary>
Motivation: 现有NIZK工具将谓词定义和输入转换分开实现，导致逻辑重复和潜在的安全漏洞风险

Method: 开发zkStruDul语言，将输入转换和谓词定义统一为单一抽象，编译器可从中投影两个过程

Result: 消除了重复代码和危险的不匹配问题，提供高级抽象并支持递归证明

Conclusion: zkStruDul通过统一抽象解决了NIZK应用中逻辑重复和安全不匹配的核心问题
...
TL;DR: Ksurf方法在云资源编排中显著降低了延迟方差，减少了CPU和内存使用，并节省了成本


<details>
  <summary>Details</summary>
Motivation: 云数据中心中资源配置参数搜索空间大且存在不确定性，现有方法在高度变化的云环境中准确性不足

Method: 将Ksurf作为上下文多臂老虎机的目标函数模型，集成到Drone编排器中处理高度可变的工作负载

Result: 延迟方差降低41%(p95)和47%(p99)，CPU使用减少4%，主节点内存使用减少7MB，平均工作pod数量节省7%成本

Conclusion: Ksurf在高度变化的云环境中能够有效优化资源编排，显著提升性能并降低成本
...
TL;DR: 本文研究雾-云计算模型中事务调度问题，提出近似算法来最小化调度总成本，针对单对象和多对象访问场景分别给出O(log n·log D)和O(k·log n·log D)的近似比。


<details>
  <summary>Details</summary>
Motivation: 在分布式系统中，事务调度对高效分配共享资源至关重要。雾-云计算模型中事务和共享对象可以在网络中移动，需要设计有效的调度策略来最小化总成本。

Method: 针对常数倍维网络，提出集中式和分布式调度算法。单对象场景下让对象移动到事务节点或事务移动到对象节点；多对象场景下考虑事务最多访问k个对象，设计相应的调度策略。

Result: 单对象调度算法达到O(log n·log D)近似比，多对象调度算法达到O(k·log n·log D)近似比，其中n为节点数，D为网络直径。

Conclusion: 提出的调度算法在理论和实际网络中都有效，分布式版本无需全局事务知识，适用于实际部署。
...
TL;DR: MoFa是一个新的预训练性能建模框架，统一了多维优化特征和容错机制，通过增强的成本模型准确捕捉关键优化效果，并基于历史集群可靠性数据集成容错模型，为LLM预训练系统设计提供先验指导。


<details>
  <summary>Details</summary>
Motivation: 随着LLM参数规模从数十亿增长到数万亿，分布式预训练需要数千到数万台设备。传统手动调优方法成本高昂，现有性能建模方法无法全面考虑常见优化特征，且忽略了检查点恢复等容错机制在长期预训练中的显著开销。

Method: 提出MoFa框架，包含增强的成本模型来准确捕捉关键优化效果，并基于历史集群可靠性数据集成容错模型。开发了基于MoFa的调优系统，探索各种场景下的最优预训练性能和潜在瓶颈。
...
TL;DR: GPU系统中存在性能变异问题，特别是在运行LLM训练时，热不平衡导致的"Lit Silicon"效应会引发节点级性能下降。通过检测和缓解技术，可实现高达6%的性能提升和4%的功耗改进。


<details>
  <summary>Details</summary>
Motivation: 现代数据中心广泛使用GPU系统，但GPU在节点和集群层面存在性能变异问题，严重影响高性能计算和AI工作负载（如大语言模型训练）的效率。

Method: 分析单节点多GPU系统运行LLM训练的性能，发现内核级性能变异与并发计算通信(C3)高度相关。提出Lit Silicon效应模型，设计检测和缓解技术，评估三种电源管理解决方案。

Result: 在两个AMD Instinct MI300X GPU系统和两个LLM训练框架上进行实验，观察到高达6%的性能提升和4%的功耗改进。

Conclusion: 提出的解决方案几乎零成本，可作为新的节点级电源管理层在数据中心中轻松部署，有望节省数亿美元成本。
...
TL;DR: CacheX通过探测VM内的缓存抽象信息，无需硬件或虚拟机管理程序支持，实现了LLC竞争感知的任务调度和虚拟颜色感知的页面缓存管理，有效提升云虚拟机中各种工作负载的缓存利用率。


<details>
  <summary>Details</summary>
Motivation: 云虚拟机中的缓存优化通常无效，因为VM对预配置缓存的可见性和控制有限，无法了解缓存配置细节，也无法通过页面放置策略影响缓存使用。

Method: 提出CacheX解决方案，使用驱逐集探测VM内准确细粒度的缓存抽象信息，并开发了LLC竞争感知任务调度和虚拟颜色感知页面缓存管理两种新技术。

Result: 在x86 Linux内核中实现的CacheX评估表明，它能有效提高公共云虚拟机中各种工作负载的缓存利用率。

Conclusion: CacheX提供了一种无需硬件或hypervisor支持的解决方案，能够有效解决云虚拟机中缓存优化面临的可见性和控制限制问题。
...
<details>
  <summary>Details</summary>
Motivation: 多服务器架构中的动态网络拥塞给延迟敏感的嵌入式应用带来了边缘服务器选择的挑战，需要一种轻量级且高效的解决方案。

Method: 使用被动测量（到达率、利用率、负载大小）和指数调制有理延迟模型，计算平衡预测延迟和可靠性的分数，确保只有在预期收益有意义时才进行切换。

Result: MO-HAN在降低平均延迟和尾部延迟方面始终优于静态和公平分配基线方法，同时将切换次数相比纯机会选择减少了近50%。

Conclusion: MO-HAN无需侵入式检测或重型学习基础设施，为资源受限的嵌入式设备提供了一种实用的边缘服务器选择方案。
...
TL;DR: 提出基于监督学习的稀疏矩阵重排序算法选择模型，通过理解矩阵特征与常用重排序算法的关联，实现自动智能选择，相比仅使用AMD算法可减少55.37%求解时间。


<details>
  <summary>Details</summary>
Motivation: 传统稀疏矩阵排序算法选择依赖暴力搜索或经验知识，缺乏适应不同稀疏矩阵结构的能力，需要更智能的自动化选择方法。

Method: 采用监督学习模型，学习矩阵特征与常用重排序算法之间的关联关系，实现算法自动选择。

Result: 在Florida稀疏矩阵数据集上测试，模型能准确预测最优重排序算法，相比仅使用AMD算法减少55.37%求解时间，平均加速比为1.45。
...
TL;DR: 本文提出了一种新颖的现代工作负载调度器分类方法，描述了操作系统进程调度器、集群系统作业调度器和大数据调度器三大类，并讨论了它们的演变历程和算法特征。


<details>
  <summary>Details</summary>
Motivation: 对现代工作负载调度器进行系统分类，理解不同类型调度器从早期采用到现代实现的发展演变过程。

Method: 通过描述三类调度器的特征、算法使用和功能，分析它们的进化历程，比较不同类别调度器之间的差异。

Result: 建立了现代工作负载调度器的分类框架，识别了三类主要调度器的特征差异和时序发展模式。

Conclusion: 尽管调度器应用于不同规模系统（本地和分布式），但在调度策略设计重点上存在相似性。
...
TL;DR: FastGraph是一种专门为低维空间（2-10维）图构建优化的GPU加速k近邻算法，相比现有方法提速20-40倍且内存开销极小。
...
Result: 在维度小于10的情况下，比FAISS、ANNOY和SCANN等最先进库快20-40倍，几乎没有内存开销。

Conclusion: FastGraph显著提升了GNN工作流程的性能，特别适用于低维计算密集型应用。
...
TL;DR: STAGE是一个能够合成高保真执行轨迹的框架，用于准确建模大语言模型工作负载，支持全面的并行化策略，可扩展到32K GPU规模。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏可扩展且表达力强的机制来建模分布式工作负载执行，且大型基础设施访问受限，现有平台轨迹难以适应未来更大规模系统配置的研究需求。

Method: 开发了符号张量图生成器(STAGE)框架，通过合成高保真执行轨迹来建模LLM工作负载，支持多种并行化策略，可系统探索各种LLM架构和系统配置。

Result: STAGE成功合成了覆盖32K GPU的高保真LLM轨迹，在计算、内存和通信方面保持了张量级精度，展示了良好的可扩展性。

Conclusion: STAGE框架为分布式机器学习系统的进一步研究提供了有力工具，将公开发布以促进相关研究。
...
TL;DR: 提出基于FPGA的历史感知轨迹k-匿名化方法，结合历史轨迹搜索和最短路径计算，在保持隐私的同时提高数据实用性
...
Method: 开发新型FPGA硬件架构，集成并行历史轨迹搜索和传统最短路径查找，使用自定义定点计数模块准确权衡历史数据贡献

Result: 实现超过6,000条记录/秒的实时吞吐量，数据保留率比先前设计提高1.2%，更有效地保留主要干线道路
...
TL;DR: AssertMiner是一个模块级断言生成框架，利用AST静态信息辅助LLM挖掘断言，解决了现有方法只生成顶层断言而忽略微架构级别模块验证需求的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于设计规范的断言生成方法通常只产生顶层断言，忽视了微架构级别模块中更频繁出现设计错误的实现细节的验证需求。

Method: 通过AST结构提取获得模块调用图、I/O表和数据流图，引导LLM生成模块级规范并挖掘模块级断言。

Result: 评估显示AssertMiner在生成模块高质量断言方面优于AssertLLM和Spec2Assertion等方法，集成后能显著提升结构覆盖率和错误检测能力。
...
TL;DR: 本文综述了AI与计算机架构的协同演进，分析了GPU、ASIC、FPGA等主流AI加速器架构的设计理念、性能权衡，以及数据流优化、内存层次结构、稀疏性和量化等核心原则，并展望了存内计算和神经形态计算等新兴技术。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型特别是深度神经网络复杂度的增加，其巨大的计算需求已使传统架构达到极限，需要研究专门优化的计算架构来加速现代AI工作负载。

Method: 通过结构化回顾AI与计算机架构的协同演进，分析主流加速器架构的设计哲学、关键特征和性能权衡，结合行业标准基准的定量性能数据进行综合评估。

Result: 展示了不同架构范式在AI加速方面的性能特征和适用场景，揭示了硬件-软件协同设计对于实现性能和能效优化的关键作用。
...
TL;DR: 本文探讨数据中心和超级计算机网络的两个关键方面：防止重负载下性能退化的拥塞控制，以及空闲时节能的电源管理，并分析两者之间的相互作用。


<details>
  <summary>Details</summary>
Motivation: 随着数据中心和超级计算机在日常生活和科学研究中的重要性日益增长，这些大规模复杂系统的网络互连可能成为整个基础设施的瓶颈。需要先进的技术和工程来确保系统正常运行，而不仅仅是依赖原始资源。

Method: 研究网络的两个关键方面：拥塞控制（防止重负载下性能退化）和电源管理（空闲时节能），并探索这两个方面之间的相互作用关系。

Result: 分析表明网络拥塞控制和电源管理是确保大规模计算系统高效运行的两个重要机制，它们之间存在相互影响的关系。

Conclusion: 在大规模计算系统中，网络拥塞控制和电源管理是相互关联的关键技术，需要综合考虑以确保系统在高负载时保持性能，在空闲时节省能源。
...
TL;DR: 本文提出了一种基于波束空间处理的复杂稀疏自适应均衡器(CSPADE)及其VLSI架构，用于毫米波大规模MIMO系统中的低功耗数据检测。


<details>
  <summary>Details</summary>
Motivation: 大规模MIMO和毫米波通信是未来无线系统的关键技术，但其部署会带来高昂的基带处理硬件成本和功耗。波束空间处理利用毫米波频段的信道稀疏性来降低基带处理复杂度。

Method: 提出了复杂稀疏自适应均衡器(CSPADE)算法及对应的VLSI架构，包括全并行实现和基于顺序乘累加(MAC)的架构。

Result: 在22nm FDSOI工艺中实现，全并行CSPADE相比天线域均衡可节省高达54%功耗，基于MAC的架构可节省高达66%功耗，同时实现了现有大规模MIMO数据检测器中最高的吞吐量。

Conclusion: 所提出的波束空间处理架构能够显著降低大规模MIMO系统的功耗，同时保持高吞吐量和良好的能效与面积效率。
