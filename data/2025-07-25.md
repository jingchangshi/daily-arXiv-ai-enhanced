...
Motivation: 随着具有定量特征（如概率）的语言兴起，需要扩展共归纳方法以支持更精细的行为一致性（如行为距离），并确保其作为程序同余性的合理性。

Method: 采用基于抽象高阶规范（AHOS）的范畴方法，将行为一致性建模为AHOS基范畴上的纤维化，提出通用的同余定理。

Result: 在自然条件下，AHOS模型中的最大行为（双）一致性形成同余性，适用于概率高阶语言的互模拟和行为伪度量。

Conclusion: 该理论为高阶语言的行为一致性提供了统一的框架，并成功应用于概率语言的互模拟和伪度量。
...
<details>
  <summary>Details</summary>
Motivation: 解决SN9中每个矿工需本地化整个模型和奖励分配不公平的问题，实现更高效的分布式预训练。

Method: 引入IOTA架构，包括数据与管道并行、连续激励、激活压缩、Butterfly All-Reduce和CLASP公平分配方案。

Result: 实现了模型规模随参与者数量扩展、通信带宽减少128倍、线性可扩展性和公平贡献评估。

Conclusion: IOTA通过协作和公平激励，显著提升了分布式预训练的效率和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有系统无法有效处理LLM应用中多样化的延迟需求，导致用户体验和调度效率低下。

Method: PolyServe将请求按延迟需求分组，动态调度到服务器子集，并通过负载梯度实现自动扩展。

Result: PolyServe相比现有策略提升了1.23倍的吞吐量，达到最优吞吐量的92.5%。

Conclusion: PolyServe通过精细的调度策略显著提升了多SLO场景下的性能和效率。
...
Method: 使用随机生成的QUBO矩阵（1000x1000至45000x45000）和六种收敛阈值，测试五种求解器的解质量和计算时间。

Result: Neal解质量最高但内存限制大；PyTorch可扩展性最好且运行时间短；JAX表现接近PyTorch但变量限制更严；SciPy表现最差。

Conclusion: PyTorch在解质量、可扩展性和运行时间上表现最均衡，适合大规模QUBO问题。
...
<details>
  <summary>Details</summary>
Motivation: 资源受限的嵌入式平台需要高效部署现代CNN，但传统硅缩放已接近极限，需通过定制硬件解决性能与功耗的权衡。

Method: 研究硬件集成挑战，提出基于RISC-V Vector 1.0扩展的灵活编程模型，优化内存层次结构和执行块布局。

Result: 实验显示图像预处理速度提升9倍，YOLOv3回退层执行速度提升3倍，功耗低于传统并行平台。

Conclusion: RISC-V Vector 1.0扩展为异构嵌入式SoC提供了高效的编程模型，平衡计算与内存占用，支持现代深度学习数据流。
...
Method: 采用FIFO、LRU和优先级缓存策略，选择性转发重要模型更新，以减少带宽使用。
...
Method: 设计模块化后端抽象层以支持多平台，并提出类别感知的单次提示方法以提高生成质量。

Result: 评估了7个先进LLM，发现任务难度差异大、泛化能力不足，但针对性提示策略有效。
...
Method: 采用基于FPGA的低功耗加速器和高速总线，结合定制操作系统VDiSK，实现即插即用的AI管道和加密数据集。

Result: 实验显示1到5个神经计算加速器的吞吐量接近线性扩展，同时揭示了基于USB3总线的性能极限。

Conclusion: CHAMP在生物识别、监控和灾难响应中有广泛应用前景，未来将优化总线协议和模块功能。
...
<details>
  <summary>Details</summary>
Motivation: 解决移动游戏和通信技术中服务管理和SLA合规性的挑战，尤其是非侵入式用户延迟测量的需求。

Method: 集成NWDAF和UPF的闭环架构，嵌入AI模型进行游戏分类和延迟感知。

Result: AI模型在NWDAF中的应用成功实现了游戏分类，为移动边缘游戏研究开辟了新途径。

Conclusion: 该架构有效提升了5G控制平面的延迟感知能力，为未来研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 大规模AI模型的指数增长导致计算和电力需求超过单一数据中心的容量，需跨地理分布站点分配训练任务，但通信开销成为挑战。

Method: PowerTrip采用动态贪婪方法，基于电力-成本启发式选择站点，优先高电力可用性和低网络延迟，优化训练效率。

Result: 使用真实Google电力数据评估，PowerTrip比现有基线策略将时间-准确率降低达50%。

Conclusion: PowerTrip有效解决了电力受限环境中的训练效率问题，显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 微服务在云平台中的共置部署提高了资源利用率，但也带来了资源竞争和干扰问题，尤其是在大规模、多样化的应用场景中。

Method: 通过分析大规模微服务集群的特性，采用CPI作为干扰度量指标，并利用多维指标实现CPI的准确预测。基于此设计了C-Koordinator平台。

Result: 干扰预测模型准确率达90.3%，应用延迟在所有百分位（P50、P90、P99）均显著降低，提升幅度达16.7%至36.1%。

Conclusion: C-Koordinator平台能有效缓解共置环境中的干扰问题，提升微服务集群的性能稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有LLM服务系统在资源管理和动态负载适应上的不足，提高资源利用率和性能。

Method: 提出模块级复制和迁移操作，开发自动扩展机制，动态调整资源分配。

Result: 成本降低46%，延迟减少14%-75%，吞吐量平均提升1.16x-4x。

Conclusion: CoCoServe在动态负载下显著提升LLM服务的效率和成本效益。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的高计算需求导致云环境中部署效率低下，传统推理服务方法存在资源浪费、高成本和可扩展性受限的问题。

Method: 采用云原生技术（如Kubernetes自动扩展）构建系统，动态适应工作负载变化，优化资源分配和性能。

Result: 实验证明，云原生架构能有效减少延迟、提高吞吐量，并适应高需求场景。

Conclusion: 云原生框架有望重塑可扩展的LLM推理服务未来，为云计算和人工智能领域提供重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 边缘视频分析（EVA）的复杂性增加，现有调度系统在动态环境中表现不佳，局部强化学习（RL）存在可扩展性和适应性不足的问题。

Method: FCPO整合CRL和FRL，动态调整推理批次大小、输入分辨率和多线程处理，通过特定代理聚合方案和多样性感知经验缓冲区实现优化。

Result: 实验显示，FCPO在有效吞吐量上提升5倍，延迟降低60%，收敛速度加快20%，内存消耗减少10倍。

Conclusion: FCPO通过结合CRL和FRL，显著提升了EVA系统的性能和适应性，为动态边缘环境提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统并行离散事件模拟（PDES）引擎在资源分配和复杂交互中存在局限性，亟需优化以提高大规模模拟的性能。

Method: 引入异步监听线程、METIS负载均衡策略、实体交互求解器和空间哈希算法四项改进。

Result: 实验验证显示，框架在时间和计算效率上显著提升，比基线实现快16倍，同步开销减少58.18%。

Conclusion: 优化框架为大规模PDES模拟提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 全球数据量快速增长，传统阈值激活机制依赖人工调整，难以适应动态变化。

Method: 使用强化学习模型，定义状态、动作和奖励，自动学习系统特性并适应工作负载。

Result: 策略在保证服务质量的同时，优化了提供商利润与环境影响的平衡。

Conclusion: 强化学习为云系统数据复制提供了动态自适应的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 玻璃中介层多芯片架构虽具优势，但系统规模增大时封装变形问题突出，传统封装技术无法有效解决，需新方法平衡性能与可靠性。

Method: 提出一种热、翘曲和性能感知的设计框架，通过表面和嵌入式芯片的协同优化，平衡性能、功耗和结构可靠性。

Result: 优化后的架构在深度神经网络任务中性能提升64.7%，功耗降低40%，同时降低制造成本。

Conclusion: 该设计框架有效解决了玻璃中介层多芯片系统的翘曲问题，实现了性能、功耗和可靠性的优化。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CPU的解决方案忽略了LLM推理中预填充和解码阶段的工作负载差异，采用静态的NUMA节点模型分区和供应商库，效果不佳。

Method: Sandwich采用硬件为中心的优化方法，为预填充和解码阶段分别设计执行计划，并在多种CPU平台上进行评估。

Result: Sandwich在吞吐量、延迟和资源需求方面显著优于现有方案，生成的GEMM内核性能接近静态编译器，但调优成本低得多。

Conclusion: Sandwich通过针对性的优化，为CPU上的LLM服务提供了高效且资源友好的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着DRAM密度增加，Rowhammer问题加剧，现有PRAC+ABO机制存在性能开销和全通道暂停的问题。

Method: 引入集中式计数器减少延迟，实现银行级粒度缓解，仅暂停受攻击的银行。

Result: 性能提升8%（最高20%），能耗降低19%，性能攻击影响限制在6%以内。

Conclusion: PRACtical在保持安全性的同时显著优化了性能和能耗。
